export default function TrustPillStream() {
  const credentials = [
    { 
    ...
    { 
      ...
      text: "Same-Day Emergencies" 
    },
  ];

  return (
    <div className="w-full bg-white dark:bg-navyslate-950 border-y border-canvas-100 dark:border-navyslate-800 py-6 overflow-hidden flex transition-colors duration-300">
      <div className="flex animate-[marquee_20s_linear_infinite] whitespace-nowrap hover:[animation-play-state:paused]">
        {[...credentials, ...credentials].map((cred, idx) => (
          <div key={idx} className="flex items-center mx-4 md:mx-8 bg-canvas-50 dark:bg-navyslate-900 px-4 py-2 rounded-full border border-canvas-100 dark:border-navyslate-800 shadow-sm transition-colors duration-300">
            <span className="text-trustcyan-500 mr-2 flex-shrink-0">{cred.icon}</span>
            <span className="text-sm font-semibold text-navyslate-800 dark:text-navyslate-200">{cred.text}</span>
          </div>
        ))}
      </div>
